...
def handle_play_playlist(call):
    ...
    Отображает список треков из выбранного плейлиста.
    ...
    for track_name in playlist_tracks:
        ...
        if track_id:
            bot.send_audio(chat_id, track_id, caption=f"🎵 {track_name}")
        else:
            bot.send_message(chat_id, f"❌ Трек '{track_name}' не найден в библиотеке.")

    bot.answer_callback_query(call.id)
    bot.send_message(chat_id, f"🎵 Плейлист '{playlist_name}' завершен.")
