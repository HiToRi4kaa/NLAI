...
def handle_edit_playlist(call):
    ...
    Видаляє старе меню та відображає повідомлення з назвою плейлиста і кнопками "Редагувати" та "Назад".
    ...
    # Створюємо кнопки
    markup = telebot.types.InlineKeyboardMarkup()
    edit_button = telebot.types.InlineKeyboardButton("✏️ Редагувати", callback_data=f'manage_playlist:{playlist_name}')
    back_button = telebot.types.InlineKeyboardButton("🔙 Назад", callback_data='edit_playlists:0')
    markup.row(edit_button, back_button)
