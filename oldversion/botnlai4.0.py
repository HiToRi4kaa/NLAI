...
def handle_manage_playlist(call):
    """
    Обробляє керування треками у плейлисті (додавання/видалення) з підтримкою пагінації.
    ...
    # Параметри пагінації
    tracks_per_page = 5
    total_pages = (len(available_tracks) + tracks_per_page - 1) // tracks_per_page
    start_index = current_page * tracks_per_page
    end_index = start_index + tracks_per_page
    page_tracks = available_tracks[start_index:end_index]

    # Створюємо кнопки для додавання/видалення треків
    markup = telebot.types.InlineKeyboardMarkup()
    for track_name in page_tracks:
        track_id = hashlib.md5(track_name.encode()).hexdigest()[:10]
    ...
    if current_page > 0:
        prev_button = telebot.types.InlineKeyboardButton("⬅️ Попередня сторінка", callback_data=f'manage_playlist:{playlist_name}:{current_page - 1}')
        markup.add(prev_button)
    if current_page < total_pages - 1:
        next_button = telebot.types.InlineKeyboardButton("➡️ Наступна сторінка", callback_data=f'manage_playlist:{playlist_name}:{current_page + 1}')
        markup.add(next_button)
    ...
    # Обробка помилок при редагуванні повідомлення
    try:
        bot.edit_message_text(
            chat_id=chat_id,
            message_id=call.message.message_id,
            text=f"🎵 Керування плейлистом '{playlist_name}' (Сторінка {current_page + 1} з {total_pages}):",
            reply_markup=markup
        )
    except Exception as e:
        logging.error(f"Помилка при редагуванні повідомлення: {e}")
        bot.answer_callback_query(call.id, "❌ Не вдалося оновити меню. Спробуйте ще раз.")
...
def handle_track_action(call):
    ...
    chat_id = call.message.chat.id
    data_parts = call.data.split(':')
    action, playlist_name, track_id = data_parts[:3]
    current_page = int(data_parts[3]) if len(data_parts) > 3 and data_parts[3].isdigit() else 0
    ...
    # Оновлюємо кнопки для управління плейлистом
    markup = telebot.types.InlineKeyboardMarkup()
    for track in page_tracks:
        track_id = hashlib.md5(track.encode()).hexdigest()[:10]
        if track in user_playlists[playlist_name]:
            button = telebot.types.InlineKeyboardButton(f"❌ Видалити {track}", callback_data=f'remove:{playlist_name}:{track_id}:{current_page}')
        else:
            button = telebot.types.InlineKeyboardButton(f"➕ Додати {track}", callback_data=f'add:{playlist_name}:{track_id}:{current_page}')
        markup.add(button)
