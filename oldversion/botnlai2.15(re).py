...
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelнем - %(message)s')
...
def generate_safe_callback_data(prefix, data):
    """
    Генерує безпечну callback_data з використанням хешування для уникнення перевищення ліміту довжини.
...
def send_welcome(message):
    ...
    if message.chat.id in user_data:
        ...
        user_data[message.chat_id]['creating_playlist'] = False
...
def handle_music_option(call):
    ...
    bot.send_audio(chat_id, current_track_id, caption=f"🎵 Відтворення: {current_track_name}", reply_markup=markup)
...
def handle_track_navigation(call):
    ...
    bot.edit_message_media(
        ...
        media=telebot.types.InputMediaAudio(media=current_track_id, caption=f"🎵 Відтворення: {current_track_name}"),
        reply_markup=markup
    )
...
def handle_view_playlists(call):
    """
    Обрабатывает просмотр плейлистов пользователя.
...
def handle_play_playlist(call):
    ...
    Args:
        call (telebot.types.CallbackQuery): Объект с данными о нажатой кнопке.
    """
    chat_id = call.message.chat.id  # Определение chat_id
    ...
    if not playlist_tracks:
        bot.answer_callback_query(call.id)
        bot.send_message(chat_id, f"🎵 Плейлист '{playlist_name}' пуст. Добавьте треки через меню редактирования.")
        return
...
def handle_invalid_callback(call):
    """
    Обрабатывает недействительные кнопки.

    Если callback_data не соответствует ни одному обработчику, бот игнорирует её.

    Args:
        call (telebot.types.CallbackQuery): Объект с данными о нажатой кнопке.
    """
    valid_callbacks = [
        'music', 'add_music', 'music_option', 'playlist', 'view_playlists',
        'create_playlist', 'edit_playlist', 'prev_track', 'next_track'
    ]
    if not any(call.data.startswith(valid) for valid in valid_callbacks):
        return
